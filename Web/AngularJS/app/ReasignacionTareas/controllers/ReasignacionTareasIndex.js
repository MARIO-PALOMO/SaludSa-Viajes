...
/**
 * Gestiona la lógica de control de la vista index del módulo Reasignacion de Tareas.
 */
app_reasignacion_tareas.controller('ReasignacionTareasIndex', [
    '$scope',
    '$rootScope',
    'DTOptionsBuilder',
    'DTColumnDefBuilder',
	...
	function ($scope, $rootScope, DTOptionsBuilder, DTColumnDefBuilder, $uibModal, $log, $http) {
		...
		$scope.ObtenerMetadatos = function () {
			$http.get($rootScope.pathURL + '/ReasignacionTareas/ObtenerMetadatos')
		...
		$scope.ObtenerTareasPorResponsable = function () {
			if ($scope.ResponsableActual && $scope.ResponsableActual.Usuario) {
				$http.get($rootScope.pathURL + '/ReasignacionTareas/ObtenerTareasPorResponsable?UsuarioResponsable=' + $scope.ResponsableActual.Usuario)
					.success(function (response) {
						var mensajes = '';
		...
		$scope.Reasignar = function () {
			...
			if (errores.length > 1) {
				...
				$http.post($rootScope.pathURL + '/ReasignacionTareas/Reasignar', { TareasReasignar: TareasReasignar, ResponsableNuevo: $scope.ResponsableNuevo.Usuario, ResponsableAnterior: $scope.ResponsableActual.Usuario })
					.success(function (response) {
						var mensajes = '';
