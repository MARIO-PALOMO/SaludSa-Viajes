...
/**
 * Gestiona la lógica de control de la vista JefeCompraProductoOtro del módulo Tarea.
 */
app_tarea.controller('TareaJefeCompraProductoOtroEdit', [
    '$scope',
    '$rootScope',
    '$uibModal',
    '$log',
	...
	function ($scope, $rootScope, $uibModal, $log, $http, json_js_datetime, Persistencia, $window) {
		...
		$scope.ValidarEnviar = function () {
			...
			angular.forEach($scope.Solicitud.Detalles, function (item) {
				...
				if (item.ProductoBien) {
					if (item.Tipo == '0' && item.ProductoBien.CodigoArticulo == 'BIEN_OTRO_P123') {
						ErrorProductoOtro = true;
					}
				}
				...
				if (item.ProductoServicio) {
					if (item.Tipo == '2' && item.ProductoServicio.CodigoArticulo == 'SERVICIO_OTRO_P123') {
						ErrorProductoOtro = true;
					}
				}
				...
				if (item.PlantillaDistribucionDetalle && item.PlantillaDistribucionDetalle.length > 0) {
					...
					angular.forEach(item.PlantillaDistribucionDetalle, function (item2) {
						if (item2.EstadoId == 1) {
							...
							PorcentajeFinal += parseFloat(item2.Porcentaje);
			...
			if (ErrorProductoOtro) {
				errores = errores + '<li>Debe corregir todos los detalles con producto "Otro".</li>';
			}
		...
		$scope.Crear = function () {
			...
			if ($scope.ValidarEnviar()) {
				...
				$http.post($rootScope.pathURL + '/Tarea/TareaJefeCompraProductoOtroEdit', {
					Detalles: Detalles,
