...
/**
 * Gestiona la lógica de control de la vista AdjuntarFacturaEdit del módulo Tarea.
 */
app_tarea.controller('TareaAdjuntarFacturaEdit', [
    '$scope',
    '$rootScope',
    '$uibModal',
    '$log',
	...
	function ($scope, $rootScope, $uibModal, $log, $http, json_js_datetime, json_js_date, $window, Persistencia) {
		...
		$scope.BuscarFacturasElectronicas = function () {
			...
			modalInstance.result.then(function (result) {
				...
				angular.forEach(result, function (item) {
					item.EstadoId = 1;
					comprobantesAdd.push(item);
				});

				$http.post($rootScope.pathURL + '/ComprobanteElectronico/Adicionar', { comprobantesAdd: comprobantesAdd, RecepcionId: $scope.Recepcion.Id })
					.success(function (response) {
						var mensajes = '';
						...
						if (mensajes.length > 1) {
							...
							$scope.Recepcion.ComprobantesElectronicos = response.comprobantes;
							toastr['success']("Comprobantes electrónicos adicionados.", "Confirmación");
						}
					});
		...
		$scope.EliminarFacturaElectronica = function (comprobante) {
			bootbox.confirm('<i class="fa fa-question-circle fa-3x text-danger" style="position: relative; top:10px;"></i> ¿Seguro que desea eliminar el elemento?', function (result) {
				if (result) {
					$http.post($rootScope.pathURL + '/ComprobanteElectronico/Eliminar', { comprobanteId: comprobante.Id, RecepcionId: $scope.Recepcion.Id })
						.success(function (response) {
							var mensajes = '';
		...
		$scope.Crear = function () {
			if ($scope.Validar()) {
				$http.post($rootScope.pathURL + '/Tarea/TareaAdjuntarFacturaEdit', { TareaId: $scope.TareaId, Observacion: ($scope.Observacion ? $scope.Observacion.toUpperCase() : null) })
					.success(function (response) {
						var mensajes = '';
