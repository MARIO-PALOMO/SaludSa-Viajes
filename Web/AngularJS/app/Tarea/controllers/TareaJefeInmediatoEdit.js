...
/**
 * Gestiona la lógica de control de la vista JefeInmediatoEdit del módulo Tarea.
 */
app_tarea.controller('TareaJefeInmediatoEdit', [
    '$scope',
    '$rootScope',
    '$uibModal',
    '$log',
	...
	function ($scope, $rootScope, $uibModal, $log, $http, json_js_datetime, $window) {
		...
		$scope.ObtenerDatos = function () {$http.get($rootScope.pathURL + '/Tarea/ObtenerDatos?SolicitudId=' + $scope.SolicitudId + '&TareaId=' + $scope.TareaId)
				.success(function (response) {
					var mensajes = '';
		...
		$scope.Crear = function () {
			if ($scope.Validar()) {
				...
				$http.post($rootScope.pathURL + '/Tarea/TareaJefeInmediatoEdit', { TareaId: $scope.TareaId, Accion: $scope.Accion, Observacion: ($scope.Observacion ? $scope.Observacion.toUpperCase() : null) })
					.success(function (response) {
						var mensajes = '';
