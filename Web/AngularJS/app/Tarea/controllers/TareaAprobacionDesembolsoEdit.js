...
/**
 * Gestiona la lógica de control de la vista AprobacionDesembolsoEdit del módulo Tarea.
 */
app_tarea.controller('TareaAprobacionDesembolsoEdit', [
    '$scope',
    '$rootScope',
    '$uibModal',
    '$log',
	...
	function ($scope, $rootScope, $uibModal, $log, $http, json_js_datetime, json_js_date, $window) {
		...
		$scope.Crear = function () {
			if ($scope.Validar()) {
				$http.post($rootScope.pathURL + '/Tarea/TareaAprobarDesembolsoEdit', { TareaId: $scope.TareaId, RecepcionId: $scope.Recepcion.Id, Accion: $scope.Accion, Observacion: ($scope.Observacion ? $scope.Observacion.toUpperCase() : null) })
					.success(function (response) {
						var mensajes = '';
