...
/**
 * Gestiona la lógica de control de la vista VerificacionPresupuestoEdit del módulo Tarea.
 */
app_tarea.controller('TareaVerificacionPresupuestoEdit', [
    '$scope',
    '$rootScope',
    '$uibModal',
    '$log',
	...
	function ($scope, $rootScope, $uibModal, $log, $http, json_js_datetime, $window) {
			...
			$http.get($rootScope.pathURL + '/SolicitudCompra/ObtenerMetadatos?SolicitudId=' + $scope.SolicitudId)
				.success(function (response) {
					...
					if (mensajes.length > 1) {
						...
						$scope.AprobacionGerenteArea = {};
						$scope.AprobacionVicePresidenteFinanciero = {};
				...
				.success(function (response) {
					...
					if (mensajes.length > 1) {
						...
						if ($scope.DataAccion == 'show') {
							...
							if ($scope.Tarea.UsuarioGerenteArea) {
								var tem = $scope.Metadatos.GerentesAreas.filter(function (item) {
									return item.Usuario == $scope.Tarea.UsuarioGerenteArea;
								});

								$scope.AprobacionGerenteArea = tem[0];
							}

							if ($scope.Tarea.UsuarioVicepresidenteFinanciero) {
								var tem = $scope.Metadatos.VicepresidentesFinancieros.filter(function (item) {
									return item.Usuario == $scope.Tarea.UsuarioVicepresidenteFinanciero;
								});

								$scope.AprobacionVicePresidenteFinanciero = tem[0];
							}
						}
					}
				});
		...
		$scope.Validar = function () {
			...
			if (!$scope.Accion || $scope.Accion == "") {
				errores = errores + '<li>No ha seleccionado una acción.</li>';
			}

			if ($scope.Accion != 'Dentro' && !$scope.Observacion) {
				errores = errores + '<li>No ha entrado un comentario.</li>';
			}

			if ($scope.Accion == 'Fuera') {
				if (!$scope.AprobacionGerenteArea || !$scope.AprobacionGerenteArea.Usuario) {
					errores = errores + '<li>No ha seleccionado un Generente de Área.</li>';
				}

				if (!$scope.AprobacionVicePresidenteFinanciero || !$scope.AprobacionVicePresidenteFinanciero.Usuario) {
					errores = errores + '<li>No ha seleccionado un Vicepresidente Financiero.</li>';
				}
			}
		...
		$scope.Crear = function () {
			if ($scope.Validar()) {
				...
				$http.post($rootScope.pathURL + '/Tarea/TareaVerificacionPresupuestoEdit', { TareaId: $scope.TareaId, Accion: $scope.Accion, Observacion: ($scope.Observacion ? $scope.Observacion.toUpperCase() : null), GerenteArea: ($scope.AprobacionGerenteArea ? ($scope.AprobacionGerenteArea.Usuario ? $scope.AprobacionGerenteArea.Usuario : null) : null), VicepresidenteFinanciero: ($scope.AprobacionVicePresidenteFinanciero ? ($scope.AprobacionVicePresidenteFinanciero.Usuario ? $scope.AprobacionVicePresidenteFinanciero.Usuario : null) : null) })
					.success(function (response) {
						var mensajes = '';
