...
/**
 * Gestiona la lógica de control de la vista AprobacionFueraPresupuestoEdit del módulo Tarea.
 */
app_tarea.controller('TareaAprobacionFueraPresupuestoEdit', [
    '$scope',
    '$rootScope',
    '$uibModal',
    '$log',
	...
	function ($scope, $rootScope, $uibModal, $log, $http, json_js_datetime, $window) {
		...
		$scope.Crear = function () {
			if ($scope.Validar()) {
				...
				if ($scope.Tipo == 14) { // APROBACION_FUERA_PRESUPUESTO_GERENTE_AREA
					$http.post($rootScope.pathURL + '/Tarea/TareaAprobacionFueraPresupuestoGerenteAreaEdit', { TareaId: $scope.TareaId, Accion: $scope.Accion, Observacion: ($scope.Observacion ? $scope.Observacion.toUpperCase() : null) })
						.success(function (response) {
							var mensajes = '';
				...
				else if ($scope.Tipo == 15) { // APROBACION_FUERA_PRESUPUESTO_VICEPRESIDENTE_FINANCIERO
					$http.post($rootScope.pathURL + '/Tarea/TareaAprobacionFueraPresupuestoVicepresidenteFinancieroEdit', { TareaId: $scope.TareaId, Accion: $scope.Accion, Observacion: ($scope.Observacion ? $scope.Observacion.toUpperCase() : null) })
						.success(function (response) {
							var mensajes = '';
