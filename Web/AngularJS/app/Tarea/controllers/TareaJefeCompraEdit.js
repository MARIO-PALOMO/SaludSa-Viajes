...
/**
 * Gestiona la lógica de control de la vista JefeCompraEdit del módulo Tarea.
 */
app_tarea.controller('TareaJefeCompraEdit', [
    '$scope',
    '$rootScope',
    '$uibModal',
    '$log',
	...
	function ($scope, $rootScope, $uibModal, $log, $http, json_js_datetime, $window) {
		...
		$scope.Crear = function () {
			if ($scope.Validar()) {
				...
				$http.post($rootScope.pathURL + '/Tarea/TareaJefeCompraEdit', { TareaId: $scope.TareaId, Accion: $scope.Accion, Observacion: ($scope.Observacion ? $scope.Observacion.toUpperCase() : null) })
					.success(function (response) {
						var mensajes = '';
