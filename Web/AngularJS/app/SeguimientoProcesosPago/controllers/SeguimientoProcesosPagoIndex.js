...
/**
 * Gestiona la lógica de control de la vista index del módulo Seguimiento de Procesos Pago.
 */
app_seguimiento_procesos_pago.controller('SeguimientoProcesosPagoIndex', [
    '$scope',
    '$rootScope',
    'DTOptionsBuilder',
    'DTColumnDefBuilder',
	...
	function ($scope, $rootScope, DTOptionsBuilder, DTColumnDefBuilder, $uibModal, $log, $http) {
		...
		$scope.ObtenerMetadatos = function () {
			$http.get($rootScope.pathURL + '/SeguimientoProcesosPago/ObtenerMetadatos')
		...
		$scope.ObtenerOriginadores = function () {
			...
			if ($scope.ValidarEmpresaSeleccionada()) {
				$http.get($rootScope.pathURL + '/SeguimientoProcesosPago/ObtenerOriginadores?EmpresaCodigo=' + $scope.Empresa.Codigo)
					.success(function (response) {
		...
		$scope.ObtenerSolicitudes = function () {
			...
			if ($scope.ValidarEmpresaSeleccionada()) {
				if ($scope.ValidarOriginadoresSeleccionados()) {
					...
					$http.post($rootScope.pathURL + '/SeguimientoProcesosPago/ObtenerSolicitudes', {
