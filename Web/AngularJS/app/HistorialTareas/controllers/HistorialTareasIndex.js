...
/**
 * Gestiona la lógica de control de la vista index del módulo Historial de Tareas.
 */
app_historial_tareas.controller('HistorialTareasIndex', [
    '$scope',
    '$rootScope',
    'DTOptionsBuilder',
    'DTColumnDefBuilder',
	...
	function ($scope, $rootScope, DTOptionsBuilder, DTColumnDefBuilder, $uibModal, $log, $http, $window) {
		...
		$scope.ObtenerMetadatos = function () {
			...
			$http.get($rootScope.pathURL + '/HistorialTareas/ObtenerMetadatos?NumeroSolicitud=' + NumeroSolicitud + '&EmpresaCodigo=' + EmpresaCodigo)
		...
		$scope.Buscar = function () {
			if ($scope.Validar()) {
				$http.get($rootScope.pathURL + '/HistorialTareas/Buscar?NumeroSolicitud=' + $scope.NumeroSolicitud + '&Empresa=' + $scope.Empresa.Codigo)
					.success(function (response) {
						var mensajes = '';
