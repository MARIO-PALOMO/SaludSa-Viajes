...
/**
 * Gestiona la lógica de control de la vista create del módulo Solicitud Compra.
 */
app_solicitud_compra.controller('SolicitudCompraCreate', [
    '$scope',
    '$rootScope',
    '$uibModal',
    '$log',
	...
	function ($scope, $rootScope, $uibModal, $log, $http, json_js_datetime, Persistencia, $window) {
		...
		$scope.Solicitud = {
			Detalles: [],
			Tareas: [],
			RequerimientosAdjuntos: [],
			RequerimientosAdjuntosPrevisualizar: [],
			MontoEstimado: '0.00',
			Frecuencia: 'Única'
		};
		...
		$scope.ObtenerMetadatos = function () {
			$http.get($rootScope.pathURL + '/SolicitudCompra/ObtenerMetadatos?SolicitudId=-1')
				.success(function (response) {
		...
		$scope.Crear = function (accion) {
			...
			if (Validacion) {
				...
				Persistencia.Create($scope.Solicitud.RequerimientosAdjuntos, Cabecera)
