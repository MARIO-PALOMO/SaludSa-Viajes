...
/**
 * Gestiona la lógica de control de la vista alcance del módulo Solicitud Compra.
 */
app_solicitud_compra.controller('SolicitudCompraAlcance', [
    '$scope',
    '$rootScope',
    '$uibModal',
    '$log',
	...
	function ($scope, $rootScope, $uibModal, $log, $http, json_js_datetime, Persistencia, $window) {
		...
		$scope.RequerimientosAdjuntosSalvados = [];
		...
		$scope.ObtenerSolicitud = function () {
			$http.get($rootScope.pathURL + '/SolicitudCompra/Detalle?Id=' + $scope.SolicitudId)
				.success(function (response) {
					var mensajes = '';
					...
					if (mensajes.length > 1) {
						...
						$scope.Solicitud.Descripcion = '(Alcance a la solicitud ' + $scope.Solicitud.NumeroSolicitud + ') ' + $scope.Solicitud.Descripcion;
						$scope.Solicitud.Tareas = [];
						$scope.Solicitud.NumeroSolicitud = null;
						$scope.Solicitud.FechaSolicitud = null;
		...
		$scope.AdicionarDistribucion = function (Detalle) {
			...
			angular.forEach($scope.Solicitud.Detalles, function (item, index) {
				if (item.Tipo == Detalle.Tipo && item.PlantillaDistribucionDetalle && item.PlantillaDistribucionDetalle.length > 0) {
					var nueva = {
						Id: item.Pk,
		...
		$scope.EliminarRequerimientoAdjuntoSalvado = function (adjunto) {
			$scope.Solicitud.RequerimientosAdjuntosSalvados = $scope.Solicitud.RequerimientosAdjuntosSalvados.filter(function (item) {
				return item.Id != adjunto.Id;
			});
		}
		...
		$scope.ValidarEnviar = function () {
			...
			if ((parseFloat($scope.Solicitud.MontoEstimado) >= 5000 || ($scope.Solicitud.AprobacionSubgerenteArea && $scope.Solicitud.AprobacionSubgerenteArea.Usuario == 'noaplica_0123')) && (!$scope.Solicitud.AprobacionGerenteArea || !$scope.Solicitud.AprobacionGerenteArea.Usuario)) {
		...
		$scope.Crear = function () {
			...
			var Validacion = $scope.ValidarEnviar();
			...
			if (Validacion) {
				...
				angular.forEach($scope.Solicitud.RequerimientosAdjuntosSalvados, function (item) {
					AdjuntosAnteriores.push(item.IdAdjunto + '>' + item.Nombre);
				});

				Persistencia.Clonar($scope.Solicitud.RequerimientosAdjuntos, Cabecera, $scope.SolicitudId, AdjuntosAnteriores.join("*"))
					.then(function (response) {
						...
						if (mensajes.length > 1) {
							...
							if ($scope.Solicitud.NumeroSolicitud) {
								$scope.Solicitud.FechaSolicitud = json_js_datetime.convertir(response.data.FechaSolicitud);
								toastr['success']("Elemento procesado correctamente. <br> <b>Número de solicitud: " + $scope.Solicitud.NumeroSolicitud + "</b>", "Confirmación");
							}
							else {
