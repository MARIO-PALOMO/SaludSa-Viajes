...
/**
 * Gestiona la lógica de control de la vista buscar_factura_electronica del módulo Solicitud Pago.
 */
app_solicitud_pago.controller('BuscarFacturasElectronicas', [
    '$scope',
    '$rootScope',
    '$uibModalInstance',
    '$log',
	...
	function ($scope, $rootScope, $uibModalInstance, $log, $http, $window, data) {
        ...
        $scope.ValidarFiltros = function () {
			...
			if (!$scope.ruc || $scope.ruc.length != 13) {
				errores = errores + '<li>Debe entrar un RUC válido.</li>';
			}
		...
		$scope.Seleccionar = function (factura) {
			$scope.Ok(factura);
		};
