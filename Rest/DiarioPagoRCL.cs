...
{
    /// <summary>
    /// Cliente rest para diarios de pagos.
    /// </summary>
    public class DiarioPagoRCL
    {
        /// <summary>
        /// Proceso para obtener los diarios de un tipo y de una compañía.
        /// </summary>
            ...
            int tipoDiario)
        {
            ...
            var reques = new RestRequest("ServicioAdministracion/api/DynamicsAx/ObtenerDiarios", Method.GET);

            reques.AddParameter("codigoCompania", codigoCompania);
            reques.AddParameter("tipoDiario", tipoDiario);
            ...
            List<DiarioPagoViewModel> items = new List<DiarioPagoViewModel>();

            var json = JsonConvert.SerializeObject(new
            {
                origin = "ServicioAdministracion/api/DynamicsAx/ObtenerDiarios",
            ...
            if (response.StatusCode != System.Net.HttpStatusCode.OK)
            {
                ...
                var error = string.Format("Error al intentar consumir el servicio web \"ServicioAdministracion/api/DynamicsAx/ObtenerDiarios\". Código de error: {0} - Descripción: {1}. " + mensaje, response.StatusCode, response.StatusDescription);

                EventLogUtil.EscribirLogErrorWeb(json, 38, sesion.OrigenLog);
            ...
            else
            {
                ...
                if (Content != null && Content.Datos != null)
                {
                    ...
                    {
                        items = JsonConvert.DeserializeObject<List<DiarioPagoViewModel>>(Content.Datos.ToString());
                    }
                }
                else
                {
                    ...
                    {
                        var error = "Error en la respuesta obtenida al consultar el servicio: /ServicioAdministracion/api/DynamicsAx/ObtenerDiarios";
                        EventLogUtil.EscribirLogErrorWeb(json, 38, sesion.OrigenLog);
                        throw new Exception(error);
                    }
                }
            ...
            EventLogUtil.EscribirLogInfoWeb(json, 38, sesion.OrigenLog);
